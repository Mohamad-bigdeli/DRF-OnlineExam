...
from ..serializers import ExamRetrieveSerializer, ExamCreateUpdateSerializer, ExamListSerializer
...
class ExamViewSet(viewsets.ModelViewSet):
    ...
    def next_question(self, request: Request, pk):
        ...
        if (not created and
            Score.objects.filter(exam_id=exam, student_id=user_id).exists()):
            return Response(
        ...
        if not created and timezone.now() >= participation.expires_at:
            return Response(
                {"detail": "the exam time is over"},status=status.HTTP_403_FORBIDDEN,
            )
        next_question = Question.objects.filter(exam_id=pk).exclude(answers__student=user_id).first()
        if not next_question:
            return Response({"detail":"you have answered all the questions"}, status=status.HTTP_403_FORBIDDEN)
