...
class TestFavoriteCategoryApi:
    ...
    def test_get_favorite_category_response_status_code_200(
    ...
    def test_create_favorite_category_response_status_code_201(
        self, api_client, common_user
    ):
        category = Category.objects.create(title="test-title")
        url = reverse("exam:api-v1:favorite-categories-list")
    ...
    def test_get_detail_favorite_category_response_status_code_200(
        self, api_client, common_user
    ):
        category = Category.objects.create(title="test-title")
        favorite_category = FavoriteCategory.objects.create(
            category=category, student=common_user
        )
        url = reverse(
            "exam:api-v1:favorite-categories-detail",
            kwargs={"pk": favorite_category.id},
        )
        api_client.force_authenticate(user=common_user)
        response = api_client.get(url)
        assert response.status_code == 200

    def test_delete_favorite_category_response_status_code_204(
        self, api_client, common_user
    ):
        category = Category.objects.create(title="test-title")
        favorite_category = FavoriteCategory.objects.create(
